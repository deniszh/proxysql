...
rm -f percona-release_0.1-4.precise_all.deb || true && \
...
if [ "`grep Ubuntu /etc/issue | awk '{print $2}' | cut -d. -f1`" == "12" ]; then
    ...
    apt-get -y autoremove && apt-get -y remove libmysqlclient-dev ibmysqlclient18 libmysqlclient18-dev libmysqlclient18.1 libmysqlclient18.1-dev
...
# copying hash.h from Percona server sources
...
# mimicking Percona libs as Mysql libs (because no MySQL 5.7 dev exist for precise)
...
# building libslave (only slave_a lib, tests are failing)
cd /opt/proxysql_mysqlbinlog/libslave/ && cmake . && make slave_a && \
# building binlog server
cd ../ && make && \
cp /opt/proxysql_mysqlbinlog/proxysql_binlog_reader /opt/proxysql/ && \
# build proxysql
cd /opt/proxysql && \
${MAKE} cleanbuild && \
